...
describe('Photo-first backend (image store + analysis + persistence)', () => {
  it('round-trips image bytes through the image store', async () => {
    ...
    try {
      ...
      const meta = await container.images.saveImage(PNG_BYTES, {
        extension: 'png',
  ...
  it('analyses colour samples + Spanish hints, never fabricating', async () => {
    ...
    try {
      ...
      expect(result.analysis.garmentType?.value).toBe('Camisa');
  ...
  it('persists a photo-first garment (metadata + photo + thumbnail) across restart', async () => {
    ...
    try {
      ...
      const original = await c1.images.saveImage(PNG_BYTES, { extension: 'png' });
      const thumb = await c1.images.saveImage(PNG_BYTES, { extension: 'webp' });
      ...
      expect(g?.metadata.style).toBe('Elegante');
